...
pub struct Day231;
impl Puzzle for Day231 {
    fn run(&self) {
        let filename = "input/day231.txt";
        ...
        for _i in 0..ROUNDS {
            ...
            if DEBUG {
                println!("Result after round {}", _i);
                draw_plan(&elves);
            }
        }
        let bounding_box = get_bounding_box(&elves);
        let result: i32 = (bounding_box.1 .0 - bounding_box.0 .0 + 1)
            * (bounding_box.1 .1 - bounding_box.0 .1 + 1)
            - num_elves;

        if DEBUG {
            println!(
                "Bounding box: {},{} to {}, {}. Width: {}, height: {}",
                bounding_box.0 .0,
                bounding_box.0 .1,
                bounding_box.1 .0,
                bounding_box.1 .1,
        ...
        println!("Day 23 - Part 1: Result is {}", result);
    }
}
