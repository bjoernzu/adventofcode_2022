...
impl Puzzle for Day201 {
    fn run(&self) {
        ...
        for line in input.lines() {
            if !line.is_empty() {
                input_numbers.push(CryptNumber{id, value: line.parse::<i64>().unwrap()});
                id += 1;
            }
        }
        ...
        for number in input_numbers {
            ...
            let future_position = (current_position as i64 + number.value + (num_numbers - 1) * 4) % (num_numbers - 1);

            let element = result_numbers.remove(current_position);
            println!("{}", future_position);
            result_numbers.insert(future_position as usize, element);
        }

        // Debug prnit to check final list
        ...
        println!("Day 20 - Part 1: Result is {}", result);
    }
}
