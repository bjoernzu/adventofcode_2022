...
impl Puzzle for Day122 {
    fn run(&self) {
        ...
        let result = find_way(&end, &start, &map);
        // Print the result
        println!("Day 12 - Part 2: Result is {}", result.last().unwrap().g_score);
    }
}
...
impl Point {
    ...
    fn calculate_distance(&mut self, target: &Point) -> usize {
        (target.height as i64 - self.height as i64).abs() as usize
    }
}
...
fn find_way(
    ...
    while !open_list.is_empty() {
        ...
        // println!(
        //     "Current point: {}, {}. gScore: {}, fScore: {}, Previous: {}, {}, Open list length: {}",
        //     current.x,
        //     current.y,
        //     current.g_score,
        //     current.f_score,
        //     came_from_list.get(&current).unwrap_or(&current).x,
        //     came_from_list.get(&current).unwrap_or(&current).y,
        //     open_list.len()
        // );
        ...
        for mut n in neighbors {
            ...
            if tentative_g_score < n.g_score && n.height + 1 >= current.height {
