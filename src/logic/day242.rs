...
fn find_way(
    ...
    pattern: usize,
    blizzard_positions: &Vec<HashSet<Position>>,
    blizzard_loop_length: usize,
    width: i32,
    height: i32,
) -> (usize, usize) {
    // Find the number
    let mut open_positions: VecDeque<(Position, usize)> = VecDeque::new();
    open_positions.push_back((starting_position.clone(), pattern));
    ...
    while !open_positions.is_empty() {
        ...
        for d in DIRECTIONS {
            ...
            if new_elves_pos == *target_position {
                return (minute + 1 - pattern, blizzard_pattern);
            }
            ...
            if new_elves_pos == *starting_position
                || (new_elves_pos.0 >= 0
                    ...
                    && !blizzard_positions[blizzard_pattern].contains(&(new_elves_pos))
                    && !closed_positions.contains(&(new_elves_pos, blizzard_pattern))
            {
                open_positions.push_back((new_elves_pos, minute + 1));
                closed_positions.insert((new_elves_pos, blizzard_pattern));
            }
        }
    }
    return (0, 0);
...
pub struct Day242;
impl Puzzle for Day242 {
    fn run(&self) {
        let filename = "input/day242.txt";
        ...
        (first, current_pattern) = find_way(
            &starting_position,
            &target_position,
            current_pattern,
            &blizzard_positions,
        ...
        (second, current_pattern) = find_way(
            &target_position,
            &starting_position,
            current_pattern,
            &blizzard_positions,
        ...
        println!("Day 24 - Part 2: Result is {}", first + second + third);
    }
}
