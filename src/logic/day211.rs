        ...
        for line in input.lines() {
            if !line.is_empty() {
                ...
                if calculation.len() == 1 {
                    monkeys.insert(id.to_string(), Monkey::Value(ValueMonkey { value: calculation[0].parse::<i64>().unwrap()}));
                }
                else {
                    monkeys.insert(id.to_string(), Monkey::Calculation(CalculationMonkey { left: calculation[0].to_string(), right: calculation[2].to_string(), operand: calculation[1].chars().nth(0).unwrap() }));
                }
